...
class Portfolio:
    ...
    def __init__(self, etf_path: str, rf_path: str, start_date: str, transaction_cost: float = 0.001):
        ...
        if start_date < self.__prices.index[0]:
            raise ValueError(
                f"Start date {start_date.date()} not found in price data. The earliest date is {self.__prices.index[0].date()}")
        if start_date > self.__prices.index[-1]:
            raise ValueError(
                f"Start date {start_date.date()} not found in price data. The latest date is {self.__prices.index[1 - 1].date()}")
        ...
        self.positions = pd.DataFrame(index=[self.current_date], columns=self.__prices.columns, data=0.0,
                                      dtype='float64')
    ...
    def advance_date(self, days: int = 1) -> None:
        ...
        for _ in range(days):
            ...
            if new_idx >= len(self.__prices):
                raise ValueError(
                    f"Cannot advance; exceeds available price data. (Last day: {self.__prices.index[current_idx].date()})")
            ...
            new_cash = (self.cash.iloc[-1] *
                        ...
                        self.visible_rf.loc[:self.current_date]["Compounded Value"].shift(1).loc[
                            self.current_date])  # cash get compounded
    ...
    def calculate_transaction_cost(self, new_weights: Dict[str, float], is_rebalance: bool = False) -> Union[
        float, Tuple[pd.Series, float, pd.Series, float, float]]:
        """
    ...
    def rebalance(self, new_weights: Dict[str, float]) -> None:
        ...
        self.cash.loc[self.current_date] = new_cash
        self.weights.loc[self.current_date] = pd.Series(new_weights).reindex(self.weights.columns,
                                                                             fill_value=0.0)  # Reflects the new allocation
...
if __name__ == "__main__":  # For testing and debugging
    ...
    weights = pd.read_csv(file_weight, index_col=0)[1500:].to_dict(
        orient='list')  # start to have valid data (non 0 weights) around 1500
