...
VARIANT_REMOVE_MOMENTUM = True # Base case: False; Variant: True # To ignore momentum return in calculation
...
def variant_case():
    ...
    elif (VARIANT_REMOVE_MOMENTUM == True & VARIANT_REMOVE_BMI == True):
        return "control_case"
...
def calculate_portfolio_weights():
    ...
    for idx, date in enumerate(df.index):
        ...
        if VARIANT_REMOVE_MOMENTUM == False: # Switch to ignore momentum return 
            ...
            current_avg_returns = pd.DataFrame({date: [1 for etf in etf_list]})
        try:
        ...
        filtered_return = current_avg_returns * momentum_flags.reshape(-1, 1)
        if filtered_return.sum().sum() <= 0:
            continue
        ...
        print(
            f"\rProcessing {date.strftime('%Y-%m-%d')}: {progress:.1%} [{'#' * int(progress * 100 + 1)}{' ' * (100 - int(progress * 100 + 1))}] Last runtime: {runtime} Total runtime: {total_runtime}",
            end="")

    # Save results and plot runtime
    filename = "weight_" + variant_case + ".csv"
    weights_df.to_csv(os.path.join(file_dir, filename))
